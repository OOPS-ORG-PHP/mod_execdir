/*
   ...
   |         This source is PHP-7.4.16 branch                             |
   +----------------------------------------------------------------------+
 */
...
PHP_FUNCTION(proc_terminate_re)
{
	...
	if ((proc = (struct php_process_handle *)zend_fetch_resource(Z_RES_P(zproc), "process", le_proc_open_re)) == NULL) {
		RETURN_FALSE;
	}
...
/* {{{ proto int proc_close_re(resource process)
   close a process opened by proc_open_re */
...
/* {{{ proto array proc_get_status_re(resource process)
   get information about a process opened by proc_open_re */
...
/* {{{ proto resource proc_open_re(string|array command, array descriptorspec, array &pipes [, string cwd [, array env [, array other_options]]])
...
PHP_FUNCTION(proc_open_re)
{
	...
	if (Z_TYPE_P(command_zv) == IS_ARRAY) {
		...
		{
			char * jcommand = get_jailed_shell_cmd (Z_STRVAL_P(command_zv));
			if ( jcommand == NULL ) {
				RETURN_FALSE;
			}
			command = pestrdup(jcommand, is_persistent);
...
# endif
	...
	efree(descriptors);
	ZVAL_RES(return_value, zend_register_resource(proc, le_proc_open_re));
	return;
...
#endif /* PHP_VERSION_ID < 80000 && PHP_VERSION_ID >= 70400 */
