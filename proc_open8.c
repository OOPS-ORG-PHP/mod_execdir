/*
   ...
   |         This source is based on PHP-8.0.3 branch                     |
   +----------------------------------------------------------------------+
 */
...
static int le_proc_open_re; /* Resource number for `proc` resources */
...
PHP_MINIT_FUNCTION(proc_open_re)
{
	le_proc_open_re = zend_register_list_destructors_ex(proc_open_rsrc_dtor, NULL, "process",
		module_number);
	return SUCCESS;
}
/* }}} */
...
PHP_FUNCTION(proc_terminate_re)
{
	...
	proc = (php_process_handle*)zend_fetch_resource(Z_RES_P(zproc), "process", le_proc_open_re);
	if (proc == NULL) {
		RETURN_THROWS();
	}
...
#ifdef HAVE_EXECDIR
		if (strlen(ZSTR_VAL(command_str)) != ZSTR_LEN(command_str)) {
			php_error_docref(NULL, E_WARNING, "NULL byte detected. Possible attack");
			RETURN_FALSE;
		}
		{
			char * jcommand = get_jailed_shell_cmd (ZSTR_VAL(command_str));
			if (jcommand == NULL) {
				RETURN_FALSE;
			}
			command = estrdup(jcommand);
...
# error You lose (configure should not have let you get here)
	...
	if (1) {
		RETVAL_RES(zend_register_resource(proc, le_proc_open_re));
	} else {
exit_fail:
		_php_free_envp(env);
		if (command) {
